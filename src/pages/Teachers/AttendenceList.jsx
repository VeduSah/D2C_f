...
const AttendanceList = () => {
  ...
  const fetchAttendance = async () => {
    try {
      ...
      const endpoint = viewMode === 'daily' 
        ...
        : 'http://localhost:8000/api/teacher-attendance/summary';
  ...
  const exportToCSV = async () => {
    ...
    try {
      const response = await axios.get('http://localhost:8000/api/teacher-attendance/csv', {
      ...
      link.href = url;
      link.setAttribute('download', `attendance_summary_${monthFilter}_${yearFilter}.csv`);
...
<div className="mb-6">
  ...
  <tr key={record.teacherId || index} className="hover:bg-gray-50">
    <td className="p-2 border">{(page - 1) * 10 + index + 1}</td>
    <td className="p-2 border">{record.name || record._id?.name}</td>
    <td className="p-2 border">{record.email || record._id?.email}</td>
    ...
    {viewMode === 'daily' ? (
      ...
      <>
        ...
        <td className="p-2 border font-medium">
          {(record.presentCount || 0) + (record.absentCount || 0) + (record.leaveCount || 0)}
        </td>
      </>
    )}
  </tr>

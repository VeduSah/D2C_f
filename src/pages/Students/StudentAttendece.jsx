...
useEffect(() => {
  const fetchTeachers = async () => {
    try {
      ...
      const assignedSections = JSON.parse(localStorage.getItem('assignedSections') || '[]');

      // Get class and section values from storage, fallback to empty string if none
      const studentClass = assignedClasses.length > 0 ? assignedClasses[0].value : '';
      const studentSection = assignedSections.length > 0 ? assignedSections[0].value : '';

      // Construct URL with filters
      const url = `https://d2-c-b.vercel.app/api/student/filter-by-class?studentClass=${studentClass}&studentSection=${studentSection}&page=1`;
  ...
  const handleSubmit = async () => {
    ...
    const submissionPromises = teachers.map(async (teacher) => {
      ...
      try {
        // Try updating
        await axios.put('http://localhost:8000/api/student-attendance/update', {
          teacher: teacher._id,
          date: selectedDate,
          status
        });
        ...
        if (error.response?.status === 404) {
          // Create if not found
          await axios.post('http://localhost:8000/api/student-attendance', {
            teacher: teacher._id,
            name: teacher.name,
            email: teacher.email,
            status,

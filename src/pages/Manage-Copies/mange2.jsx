...
import { FaCheckCircle } from "react-icons/fa"; // Added for green tick
...
const mange2 = () => {
  ...
  const assignUser = JSON.parse(localStorage.getItem("User")) || null;
  console.log("Current user:", assignUser);

  // Initialize pending actions from localStorage
  const [pendingActions, setPendingActions] = useState(() => {
    const saved = localStorage.getItem("pendingCopyActions");
    return saved ? JSON.parse(saved) : [];
  });

  // Save pending actions to localStorage whenever they change
  useEffect(() => {
    localStorage.setItem("pendingCopyActions", JSON.stringify(pendingActions));
  }, [pendingActions]);

  // Merge copyRes with today's date
  ...
  // Filter copyRes by subject and date
  ...
  // Send pending actions to backend on mount
  useEffect(() => {
    const sendPendingActions = async () => {
      ...
      try {
        for (const action of pendingActions) {
          await axios.post(`https://d2-c-b.vercel.app/api/copy-check`, action, {
            headers: {
        ...
        setPendingActions([]);
        localStorage.removeItem("pendingCopyActions");
        fetchAllCopies();
      } catch (error) {
        toast.error("Error syncing pending actions");
      } finally {
        setLoading(false);
      }
    };
          ...
          {visiblePages?.map((number) => (
            <li key={number} className="page-item">
              <button
                ...
                className={`${currentPage === number ? "bg-gray-400 text-white" : ""} px-4 py-2 mx-1 rounded-md`}
                type="button"
              >
                {number}
              </button>
  ...
  const handleCopySubmit = (data, event) => {
    if (event) {
    ...
    const isChecked = !data.copyRes[0]?.isCopyChecked;
    const dataObj = {
      studentId: data._id,
      date: date,
      subject: subject,
              ...
              copyRes: [
                {
                  ...
                  isCopyChecked: isChecked,
                  checkedByTeacher:
                    isChecked && assignUser.role === "Teacher"
                      ? assignUser.name
                      : student.copyRes[0]?.checkedByTeacher,
                  checkedByCoordinator:
                    isChecked && assignUser.role !== "Teacher"
                      ? assignUser.name
                      : student.copyRes[0]?.checkedByCoordinator,
                },
              ],
            }
          : student
    ...
    setPendingActions((prev) => [...prev, dataObj]);

    toast.success(`${typeOf} ${isChecked ? "checked" : "unchecked"} for ${data.name}`);
  };
  ...
  return (
    <>
      ...
      <div className="flex justify-end font-semibold mt-4">Date : {getDate}</div>
        ...
        {!loading ? (
          <>
            {userData ? (
                  ...
                  <tbody className="text-gray-600 divide-y">
                    {userData?.map((item, idx) => (
                      <tr key={item._id}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">{subject || "Select a Subject"}</td>
                        <td className="px-6 py-4 whitespace-nowrap">{typeOf || "Select a Type"}</td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          {item?.copyRes[0]?.checkedByTeacher ? (
                            <span className="badge badge-success badge-md text-white flex items-center gap-1">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          {role !== "Admin" && (
                            <button
                              onClick={(e) => handleCopySubmit(item, e)}
                              className="btn btn-outline btn-xs"
                              type="button"
                            >
                              {item.copyRes[0]?.isCopyChecked ? "Un-Check" : "Check"}
                            </button>
                          )}
                        </td>
                      </tr>
              ...
              <div className="flex justify-center py-4 font-semibold">No Student Data !</div>
            )}
          </>
        ) : (
          <>
